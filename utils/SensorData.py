...
class SensorData:
    ...
    def export_depth_images(self, output_path, image_size=None, frame_skip=1):
        ...
        for f in range(0, len(self.frames), frame_skip):
            ...
            with open(
                os.path.join(output_path, f"{f:04d}" + ".png"), "wb"
            ) as f:  # write 16-bit
                writer = png.Writer(
    ...
    def export_poses(self, output_path, frame_skip=1):
        ...
        for f in range(0, len(self.frames), frame_skip):
            self.save_mat_to_file(
                ...
                os.path.join(output_path, f"{f:04d}" + ".txt"),
            )
    ...
    def export_intrinsics(self, output_path, image_size=None):
        ...
        if image_size is not None:
            resize_scale = max(
                image_size[0] / self.color_height, image_size[1] / self.color_width
            )
            intrinsic_color *= resize_scale
            intrinsic_color[[2, 3], [2, 3]] = 1
