...
if __name__ == "__main__":
    ...
    for name in score_name:
        ...
        if name == "psnr":
            print(
                f"avg {name:>5} = {np.mean(metrics):.3f}, > 15 = {np.mean(metrics > 15):.3f}, > 20 = {np.mean(metrics > 20):.3f}, > 25 = {np.mean(metrics > 25):.3f}, max = {np.max(metrics):.3f}, min = {np.min(metrics):.3f}"
            )
        else:
            print(
